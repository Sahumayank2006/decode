...
export function ChartRenderer({ chart }: { chart: CanonicalChart }) {
  ...
  // Honest empty state — never silently render an axis-less blank plot.
  // If you see this in the app, the bug is upstream in extraction, not here.
  if (chart.series.length === 0 || chart.data.length === 0) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-red-300 bg-red-50/40 px-6 text-center">
        <p className="text-sm font-medium text-red-700">
          No usable series data was extracted for this chart.
        </p>
        <p className="text-xs text-red-500">
          series: {chart.series.length} found · data points: {chart.data.length} · confidence: {Math.round(chart.confidence * 100)}%
        </p>
        <button className="mt-1 rounded-md border border-red-300 px-3 py-1 text-xs text-red-700 hover:bg-red-100">
          Re-extract this chart
        </button>
      </div>
    );
  }
